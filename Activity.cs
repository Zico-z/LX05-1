    ...
    public class Activity
    {
        ...
        public IEnumerable<Model.Activity> GetVerifyPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                sql += "select * from a where num between (@pageIndex-1)*@pageSize+1 and @pageIndex*@plageSize;";
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
               string sql= "select * from activity where activityVerifyo='审查通过' and endTime>=now() order by endTime desc limit 2";
        ...
        public IEnumerable<Model.Activity> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerifyo='审查通过' and recommend='是' and endTime>=now() order by endTime desc limit 1";
        ...
        public Model.Activity GetEnd()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerifyo='审查通过' and endTime>=now() order by endTime desc limit 1";
                return cn.QueryFirstOrDefault<Model.Activity>(sql);
            }
        }
        public IEnumerable<Model.Activity> GetActivityNames()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activityId,activityName from activity";
                return cn.Query<Model.Activity>(sql);
            }
        }
        public int GetCount()
